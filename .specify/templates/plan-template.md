...
**代码质量门控**: [例如: lint, format, typecheck, build, review checklist 或 NEEDS CLARIFICATION]
...
**用户体验影响**: [例如: 受影响旅程, 关键状态, 断点, 可访问性要求 或 NEEDS CLARIFICATION]
**性能预算**: [领域特定, 例如: p95 < 200ms, 首屏 < 2s, 60 fps 或 NEEDS CLARIFICATION]
...
- [ ] **代码质量优先**: 已列出格式化、静态检查、类型检查、构建和公共接口
  契约门控, 并说明需清理的技术债或例外。
- [ ] **测试标准不可协商**: 已为每个用户故事定义必需的单元、集成、合约
  和回归测试, 并记录任何例外的原因、风险和到期时间。
- [ ] **用户体验一致性**: 已定义受影响用户旅程、加载/空状态/错误/成功/
  权限状态、响应式要求和可访问性验证方式。
- [ ] **性能要求前置**: 已声明性能预算、当前基线、验证方法和失败阈值;
  如有违规, 已在复杂度跟踪中记录理由和缓解措施。
