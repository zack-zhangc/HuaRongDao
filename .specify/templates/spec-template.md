...
<!--
  ...
  每个用户故事还必须说明如何保持与现有体验一致, 并覆盖加载、空状态、
  错误、成功和权限受限等关键状态.
...
**体验一致性**: [说明需复用的现有交互/文案/视觉模式, 以及该故事的加载、
空状态、错误、成功和权限受限状态]
...
- **UX-001**: 系统必须复用 [现有导航/组件/文案/交互] 模式, 并记录任何批准
  的偏离。
- **UX-002**: 受影响流程必须定义加载、空状态、错误、成功和权限受限状态。
- **UX-003**: 系统必须在 [目标设备/断点] 保持一致行为并满足 [可访问性要求]。

### 性能要求

- **PR-001**: 系统必须满足 [可度量预算, 例如: "关键操作 p95 < 200ms" 或
  "首屏 < 2s"]。
- **PR-002**: 团队必须定义验证方法 [例如: 自动化基准, 分析脚本, 可复现
  压测]。
- **PR-003**: 系统不得使 [既有基线/关键指标] 退化超过 [阈值]。
...
- **SC-002**: [可衡量的指标, 例如: "95% 的关键流程在目标设备上于 2 秒内
  出现首个可见反馈"]
- **SC-003**: [用户满意度指标, 例如: "90% 的用户在首次尝试时无需额外指导
  即可完成主要任务"]
